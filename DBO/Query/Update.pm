package DBO::Query::Update;

=head1 NAME

DBO::Query::Update - An SQL UPDATE wrapper
...
sub init
{
    ...
    $self->_column_names([]);
    $self->_values([]);
    $self->where_obj(new DBO::Query::Expr());
...
sub set
{
    ...
    if (@_ == 1){
        ...
        if (UNIVERSAL::isa($value,'DBO::Query::Expr')){
...
{
    my $self = shift;
    $self->where_obj->append( new DBO::Query::Expr(@_));
    return $self;
}
...
sub sql
{
    ...
    $sql .= join ", ", map {
                    UNIVERSAL::isa($values->[$_],'DBO::Query::Expr')
...
sub bind_values
{
    ...
    for (@{$self->_values}, @{$self->where_obj->bind_values}){
        if    (UNIVERSAL::isa($_,'DBO::Value'))       { push @$bv, $_->value }
