package DBO::Query::Delete;

=head1 NAME

DBO::Query::Delete - An SQL DELETE wrapper
...
sub init
{
    ...
    for(@_){
        if (UNIVERSAL::isa($_,'DBI::db')){
            ...
            croak "unrecognized argument to DBO::Query::Delete->new";
        }
    }
}
...
sub from
{
    ...
    $self->table_name(new DBO::Query::Text( $table_name ));
    return $self;
}
