package DBO::Query::Insert;

=head1 NAME

DBO::Query::Insert - An SQL INSERT wrapper.
...
sub init
{
    ...
    for ( map{ UNIVERSAL::isa($_, 'ARRAY') ? (@$_) : $_ }  @_){
        ...
        elsif (UNIVERSAL::isa($_,'DBO::Meta::TableInfo')){
            $self->into($_);
        }
        else{
            $self->columns($_);
...
sub columns
{
    ...
    for (@a){
        ...
        push @{$self->_sql_values}, DBO::Query::PlaceHolder();
    }
    return $self;
}
...
sub into
{
    ...
    if (UNIVERSAL::isa($table,'DBO::Meta::TableInfo')){
...
sub values
{
    ...
    for (@a){
        assert( (not defined $_)
                || !ref($_)
                || UNIVERSAL::isa($_,'DBO::Value')
                || UNIVERSAL::isa($_,'DBO::Query::Expr')
                || UNIVERSAL::isa($_,'DBO::Query')
                || $_ eq DBO::Query::PlaceHolder(),
                'values() must be scalars, PlaceHolders, Values, or Expr objects');

        next if (defined($_) && $_ eq DBO::Query::PlaceHolder());
        if (UNIVERSAL::isa($_, 'DBO::Query::Expr')){
...
sub sql
{
    ...
    assert(defined($dbh), 'DBO::Query::Insert::sql method requires a dbh');
...
sub bind_values
{
    ...
    for (@{$self->_values}){
        if    (UNIVERSAL::isa($_,'DBO::Value::Column')) { push @$bv, $_->get_value_for_update; }
        elsif (UNIVERSAL::isa($_,'DBO::Query::Expr'))   { push @$bv, @{$_->bind_values} }
        else  { push @$bv, $_ }
    }
