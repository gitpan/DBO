package DBO::Base::Private;

=head1 NAME

DBO::Base::Private -  The private API for DBO::Base
...
use DBO::GluedQuery;
use DBO::GluedUpdate;
use DBO::Query::Update;
use DBO::Query::Insert;
use DBO::Query::Delete;
use DBO::Value::Column;
use DBO::Util;
use DBO::Tie::PrimaryKeyList;
use base qw(DBO::DBH::Owner DBO::Base::Defs);
...
sub _dbo_column_accessor
{
    ...
    my $col = shift;
        ...
        assert(UNIVERSAL::isa($col,'DBO::Meta::ColumnInfo'), 'column accessor requires a column info object');
...
sub _dbo_column_accessor_set
{
    ...
    assert(UNIVERSAL::isa($cinfo,'DBO::Meta::ColumnInfo'), 'column accessor requires a column info object');
...
sub _dbo_has_a_column_accessor_set
{
    ...
    return if $v->has_value && DBO::Util::is_same_value($v->value, $data);
...
sub _dbo_has_a_accessor_get
{
    ...
    my $new_object = $f_class->new(new DBO::Value::Key($one_key,$key));
...
sub _dbo_has_query_accessor
{
    my $self = shift;
    my $has_query = shift;
        assert(UNIVERSAL::isa($has_query, 'DBO::Meta::HasQuery'),'_dbo_has_query_accessor requires a query object');
...
sub _dbo_has_query_accessor_get
{
    ...
    my @arr;
    tie @arr, 'DBO::Tie::PrimaryKeyList', $obj_class, $keys_list;
...
sub _dbo_has_many_accessor
{
    my $self = shift;
    my $has_many = shift;
        assert(UNIVERSAL::isa($has_many, 'DBO::Meta::HasMany'),'_dbo_has_many_accessor requires a has_many object');
...
sub _dbo_has_many_accessor_get
{
    ...
    my $has_many = shift;
        ...
        assert(UNIVERSAL::isa($has_many, 'DBO::Meta::HasMany'),'_dbo_has_many_accessor_get requires a has_many object');
...
sub _dbo_has_many_accessor_set
{
    ...
    assert(UNIVERSAL::isa($has_many, 'DBO::Meta::HasMany'),'_dbo_has_many_accessor_set requires a has_many object');
...
sub _dbo_set_primary_key
{
    ...
    if (UNIVERSAL::isa($pk_val, 'DBO::Value::Key')){
    ...
    elsif (UNIVERSAL::isa($pk_val, 'DBO::Value::Column')){ die "Not yet implemented" }
...
sub _dbo_fetch_columns
{
    my ($self,$columns) = @_;
        ...
        for(@$columns){
            assert(UNIVERSAL::isa($_,'DBO::Meta::ColumnInfo'), 'requires a columninfo object');
        }

    my $query = new DBO::GluedQuery($self->_dbo_dbh, $self, $columns);
...
sub _dbo_dbh
{
    ...
    ## Try through the DBO global class
    if (defined($dbh = DBO->dbh)){
        return $dbh;
    }
...
sub _dbo_delete_static
{
    ...
    return new DBO::Query::Delete($class->_dbo_dbh)
...
sub _dbo_insert_static
{
    ...
    if ($hash){
        ...
        push @args, $columns;
        return new DBO::Query::Insert(@args)->into($class->_dbo_table_info->name)
                ->values($values);
    }
...
sub _dbo_insert
{
    ...
    my $insert = new DBO::Query::Insert($self->_dbo_dbh,$cols);
...
sub _dbo_update_static
{
    ...
    my $update = new DBO::Query::Update(@args)->table($class->_dbo_table_info->name);
...
sub _dbo_update
{
    ...
    my $update = new DBO::GluedUpdate($self,$self->_dbo_dbh,$cols);
