...
use Carp::Assert;
use DBO::Generator qw(gen_accessor);
use DBO::Meta::TableInfo;
use DBO::Meta::ColumnInfo;
use DBO::Meta::HasA;
use DBO::Meta::HasMany;
use DBO::Meta::HasQuery;
use DBO::Meta::OneToMany;
...
sub  def_has_a        {  my @args = @_; DBO->do_after_resolve(\&_dbo_def_has_a,\@args,[caller(0)]) }
sub  def_has_many     {  my @args = @_; DBO->do_after_resolve(\&_dbo_def_has_many,\@args,[caller(0)]) }
...
# TODO: $database not used
    assert(UNIVERSAL::isa($class,'DBO::Base'), 'inherited from DBO::Base');

    my $tinfo = $class->_dbo_table_info(DBO->tables->{$table} = DBO::Meta::TableInfo->factory_new($table, $class));
}
...
sub _dbo_def_accessor
{
    ...
    my $cinfo = new DBO::Meta::ColumnInfo($tinfo, $opts->{'column'}, $opts);
...
sub _dbo_def_key
{
    ...
    ## create the key
    my $key = new DBO::Meta::Key($columns_list, $opts);
...
sub _dbo_def_has_a
{
    ...
    if (@_ == 1){
        $f_table = DBO->resolve_table_name(undef,shift);
        assert(UNIVERSAL::isa($f_table,'DBO::Meta::TableInfo'), 'foreign table name failed to resolve');
        ...
        $f_table = DBO->resolve_table_name(undef,$f_table);
        assert(UNIVERSAL::isa($f_table,'DBO::Meta::TableInfo'), 'foreign table name failed to resolve');
        ...
        $one_to_many = new DBO::Meta::OneToMany($one_key,$many_key);
    }

    my $has_a = new DBO::Meta::HasA($one_to_many, $opts);
...
sub _dbo_def_has_many
{
    ...
    return $class->_dbo_def_has_query(@_) if UNIVERSAL::isa($_[2],'DBO::Query');
    ...
    $f_table = DBO->resolve_table_name(undef, $f_table);
    assert(UNIVERSAL::isa($f_table,'DBO::Meta::TableInfo'), "foreign table name '$f_table' failed to resolve");
    ...
    my $has_many = new DBO::Meta::HasMany($one_to_many, $opts);
...
sub _dbo_def_has_query
{
    ...
    my $has_query = new DBO::Meta::HasQuery($tinfo, $f_table, $query, [@bind_subs], $opts);

    $has_query->resolve;
    assert(UNIVERSAL::isa($has_query->f_table,'DBO::Meta::TableInfo'), "f_table name '$f_table' failed to resolve");
...
sub _dbo_promote_columns
{
    ...
    for my $c (@$columns_list) {
        unless (UNIVERSAL::isa($c,'DBO::Meta::ColumnInfo')){
