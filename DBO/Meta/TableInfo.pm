...
sub factory_new
{
    ...
    return exists(DBO->tables->{$table})
            ?     DBO->tables->{$table}
...
sub add_select_group
{
    my ($self,$group,$column_list) = @_;
        ...
        for (@$column_list){
            assert(UNIVERSAL::isa($_,'DBO::Meta::ColumnInfo'));
        }
...
{
    my ($self, $f_table) = @_;
        assert(UNIVERSAL::isa($f_table,'DBO::Meta::TableInfo'), 'guess_one_to_many requires a table info');
    ...
    return new DBO::Meta::OneToMany(@{$ok[0]});
}
