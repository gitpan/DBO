...
BEGIN {
        ...
        use_ok('DBO');
        use_ok('DBO::Query');
        use_ok('DBO::Query::Delete');
        use_ok('DBO::Test::Util',qw(:all));
};

## Connect
my ($dbh,$q, $ix, $delete);
ok($dbh = $DBO::Test::Util::dbh = DBI->connect(undef,undef,undef,+{RaiseError=>1, PrintError=>1}), 'connect to database');
...
## Add data, then delete it.
    ...
    ## delete everyone!
    $delete = new DBO::Query::Delete->from('foo');
    $delete->delete($dbh);
    ...
    ## delete blythe
    $delete = new DBO::Query::Delete($dbh)->from('foo')->where(+{'name' => '?'});
