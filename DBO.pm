...
use strict;
use warnings;
use DBO::Util qw(is_same_value);
use DBO::Query;
use Carp::Assert;
use Carp qw(cluck);
use base qw(DBO::DBH::Owner);
...
sub PlaceHolder { DBO::Query::PlaceHolder() }
...
sub resolve
{
    ...
    if (defined $class->did_resolve){
        ...
        cluck("DBO::resolve was already called. The first time was from $package\:\:$subroutine at $filename line $line ");
    }
...
DBO is a mapping from database tables (or joins) to perl objects. The object accessors are columns
in the table (or join).

There are currently two layers to DBO, described below.

=head2 LAYER 1 - DBO::Query

The DBO::Query layer is a set of simple object wrappers around standard SQL
...
When used alone (without LAYER 2), the DBO::Query objects offers a few
advantages over straight DBI calls:

=head3 Advantages of using DBO::Query over DBI
...
However, the real power comes in Layer 2, where DBO methods return partially constructed
DBO::Query objects.


=head2 LAYER 2 - DBO::Base

The DBO::Base layer consists of a base class (DBO::Base) and some glue to the
DBO::Query layer.  The base class is intended to be subclassed for each table,
...
The layer 2 objects make extensive use of DBO::Query (Layer 1) objects. Many
Layer 2 methods returns partially constructed DBO::Query objects. For example,
...
like a layer 1 DBO::Query object, but it fetches Customer I<objects>, not rows.
Second, the query can be cached and reused just like a DBI prepared statement
handle.

=head1 API GUIDELINES

These are the guidelines adhered to by the DBO public API. Any exceptions you
discover are bugs.
...
should be no surprises from DBO.

=item 4. DBO objects never do more than you expect them to do.

It should be clear from your code whether it triggers a database action.  For
example, if you do not explicitly modify values in a DBO object by calling
insert(), update(), delete(), etc., then the database should not
be modified. The inverse is also true.

=item 5. All DBO objects have a dbh() method.
...
DBO::dbh(). The idea is that you need only set the $dbh handle once in your
code.

=item 6. A DBI handle ($dbh) can be passed to most methods.

Wherever sensible, DBO::Query and DBO::Base methods will accept a $dbh as an
...
using all DBO subclasses -- typically upon startup. The main purpose is to
cross-reference all your multi-table relationships (e.g., has_a, has_many).
DBO->resolve() will croak if it fails.
...
DBO will call the init() method from within new(). The default init() method is empty, and is meant
...
Where @args contains the arguments that were passed to new() that DBO::Base did not
recognize. e.g., if you pass a $dbh to new(), then it will not be passed to
through init() because DBO snatched it up.
...
is fetched. When called as a class method, it returns a DBO::Query::Insert object.
...
is raised.  When called as a class method, it returns a DBO::Query::Update
object.
...
Since your module ISA DBO::Base, it inherits all of DBO::Base's
...
The DBO layer does not increase the asymptotic running time of your
code as compared to straight DBI. That should keep most of you happy.

For the rest of you speed freaks, here are the ways that DBO optimizes for
performance:
